                        ...
                        for (int i = 0; i < categoryList.Count; i++)
                        {
                            ...
                            if (categoryList[i].Parent == null)
                            {
                                nodes.Add(new JsTreeModel() { 
                                    id = categoryList[i].CategoryId.ToString(), parent = "#",
                                    text = categoryList[i].CategoryName + "" + categoryList[i].Template + "" 
                                });
                            }
                            else
                            {
                                nodes.Add(new JsTreeModel() {
                                    id = categoryList[i].CategoryId.ToString(), 
                                    parent = categoryList[i].Parent.ToString(), 
                                    text = categoryList[i].CategoryName + "" + categoryList[i].Template + "" 
                                });
                            }
                        }
                ...
                using (var response = await client.SendAsync(request))
                {
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        for (int i = 0; i < categoryList.Count; i++)
                        {
                            ...
                            if (categoryList[i].Parent == null)
                            {
                                nodes.Add(new JsTreeModel() { id = categoryList[i].CategoryId.ToString(), parent = "#", text = categoryList[i].CategoryName + "" + categoryList[i].Template + "" });
                            }
                            else
                            {

                                nodes.Add(new JsTreeModel() { id = categoryList[i].CategoryId.ToString(), parent = categoryList[i].Parent.ToString(), text = categoryList[i].CategoryName + "" + categoryList[i].Template + "" });
                            }
                        }
