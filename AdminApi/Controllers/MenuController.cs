    ...
    public class MenuController : ControllerBase
    {
        ...
        public ActionResult GetAppMenu(int roleId)
        {
            try
            {
                ...
                var assignMenus = (from mm in _context.MenuGroupWiseMenuMapping
...
on mm.MenuId equals m.MenuID
                ...
                var parentMenus = from a in assignMenus
                                  join m in _context.Menu
on a.ParentID equals m.MenuID
        ...
        public string GetMenuAccessList(int menuGroupId)
        {
            try
            {
                ...
                var parentMenus = (from a in childMenus
                                   join m in _context.Menu
on a.ParentID equals m.MenuID
                                   select new
                                   {
                ...
                List<MenuDisplay> menuList = list.Select(s => new MenuDisplay
                {
                    MenuID = s.MenuID,
                    ParentID = s.ParentID,
                    MenuTitle = s.MenuTitle,
                    URL = s.URL,
                    IsSubMenu = s.IsSubMenu,
        ...
        public ActionResult MenuAssign(MenuOperation model)
        {
            try
            {
                ...
                if (model.Operation == "insert")
                {
                    if (objCheck == null)
                    {
                        var objAssign = new MenuGroupWiseMenuMapping
                        {
                            MenuGroupId = model.MenuGroupId,
                            MenuId = model.MenuId,
                            IsActive = true,
